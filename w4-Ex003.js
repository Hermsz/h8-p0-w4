...
//W4-003 - Logic Challenge - Mencari Median
...
function cariMedian(arr) {
  ...
  if( arr.length % 2 !== 0) {
    return oddMedian += arr[Math.floor(arr.length/2)]
  } else {
    return evenMedian += ( ( arr[Math.floor((arr.length - 1)/2)] + arr[Math.ceil(arr.length/2)] ) / 2 )
  }
}
