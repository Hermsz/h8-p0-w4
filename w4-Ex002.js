...
function fpb(angka1, angka2) {
  ...
  for( var i = angka1; i >= 2; i--) {
    if( angka1 % i === 0 ) { //Kondisi dmn Angka1 di modulus i = 0
      factorAngka1.push(i)
    }
  }

  for( var j = angka2; j >= 2; j-- ) {
    if( angka2 % j === 0 ) { //Kondisi dmn Angka2 di modulus j = 0
  ...
  for( var k = 0; k <= factorAngka1.length - 1; k++ ) {
    for( var l = 0; l <= factorAngka2.length - 1; l++) {
      if( factorAngka1[k] === factorAngka2[l]) { // kalau ada angka yang sama di push
